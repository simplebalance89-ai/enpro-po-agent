...
_p21_ping_cache = {"result": None, "expires_at": 0}

async def _check_p21_status() -> str:
    if not settings.p21_base_url:
        return "not_configured"
    now = time.time()
    if _p21_ping_cache["expires_at"] > now:
        return _p21_ping_cache["result"]
    try:
        client = P21ApiClient(
        ...
        status = "unreachable"
    _p21_ping_cache["result"] = status
    _p21_ping_cache["expires_at"] = now + 60
    return status
...
async def health():
    ...
    disk_info = {}
    try:
        du = shutil.disk_usage("/app/data" if os.path.isdir("/app/data") else ".")
        disk_info = {
    ...
    p21_status = await _check_p21_status()
...
async def _process_po_to_so(header, lines, raw_content, source, fmt) -> dict:
    ...
    result_data = {
        ...
        "customer_match": {
            ...
            "shipto_score": cust_match.shipto_score if hasattr(cust_match, "shipto_score") else 0.0,
...
async def submit_p21_single(intake_id: str, req: P21SubmitRequest, request: Request, _auth=Depends(_require_api_key)):
    ...
    actor = request.headers.get("x-actor-name", req.reviewer or "system")
    await _log_audit(intake_id, "p21_submit", actor, f"SO {so_number or 'failed'}")
...
async def _approve_po(intake_id: str, reviewer: str = "bulk", notes: str = "") -> dict:
    ...
    await _log_audit(intake_id, "approve", reviewer, notes)
...
async def approve_po(intake_id: str, req: ApproveRequest, request: Request, _auth=Depends(_require_api_key)):
    """Approve a PO -- triggers learning loop and submits to P21 (API or CISM fallback)."""
    actor = request.headers.get("x-actor-name", req.reviewer or "system")
    return await _approve_po(intake_id, reviewer=actor, notes=req.notes)


@app.post("/api/v1/review/bulk-approve")
async def bulk_approve_greens(request: Request, _auth=Depends(_require_api_key)):
    ...
    actor = request.headers.get("x-actor-name", "bulk")
    for po in eligible:
        intake_id = po.get("intake_id")
        po_no = po.get("po_no") or po.get("header", {}).get("po_no", intake_id)
        try:
            await _approve_po(intake_id, reviewer=actor)
            approved_count += 1
            po_nos.append(po_no)
    ...
    await _log_audit("", "bulk_approve", actor, f"Approved {approved_count} POs: {', '.join(po_nos)}")
...
async def reject_po(intake_id: str, req: RejectRequest, request: Request, _auth=Depends(_require_api_key)):
    ...
    await _log_audit(intake_id, "reject", actor, req.reason)
...
async def edit_po(intake_id: str, req: EditPORequest, request: Request, _auth=Depends(_require_api_key)):
    ...
    actor = request.headers.get("x-actor-name", "system")
    await _log_audit(intake_id, "edit", actor, f"Confidence now {new_conf.overall}")
...
async def load_demo_data():
    """Load sample POs into the queue for demo/testing."""
    demo_pos = [
        {
            "intake_id": "DEMO_GREEN_001",
            "po_no": "4500819454",
            "source": "ariba",
            "confidence": "green",
            ...
            "header": {
                ...
                "buyer": "Sarah Johnson",
                "ship2_name": "Waupaca Foundry, Inc.",
                "ship2_add1": "9856 State Highway 66",
                "ship2_city": "Tell City",
                "ship2_state": "IN",
                "ship2_zip": "47586",
                "ship2_country": "US",
                "customer_id_p21": "207620",
                "customer_name_p21": "Stepan Chemical",
                "comments": "UPS Ground Collect. Dock hours 7am-3pm M-F.",
            },
            "customer_match": {
                "p21_id": "207620",
                "p21_customer_name": "Stepan Chemical",
                "match_score": 0.98,
                "method": "exact",
            },
            "lines": [
                {
                    "line_no": 1,
                    "supplier_part_id": "PH0260",
                    "item_id_p21": "7931",
                    "item_description": "Pall Filter Element - High Pressure",
                    "qty_ordered": 24,
                    "unit_price": 185.50,
                    "unit_of_measure": "EA",
                    "crosswalk_match_score": 0.97,
                    "confidence": "green",
                },
                {
                    "line_no": 2,
                    "supplier_part_id": "9000238",
                    "item_id_p21": "7932",
                    "item_description": "Pall Seal Kit - Standard",
                    "qty_ordered": 12,
                    "unit_price": 96.52,
                    "unit_of_measure": "EA",
                    "crosswalk_match_score": 0.95,
                    "confidence": "green",
                },
            ],
        ...
        },
        {
            ...
            "header": {
                ...
                "buyer": "Mike Torres",
                "ship2_name": "Industrial Pump Solutions LLC",
                "ship2_add1": "4500 Commerce Pkwy",
                "ship2_city": "Houston",
                "ship2_state": "TX",
                "ship2_zip": "77001",
                "ship2_country": "US",
                "customer_id_p21": "208901",
                "customer_name_p21": "Industrial Pump Solutions",
                "comments": "",
            },
            "customer_match": {
                "p21_id": "208901",
                "p21_customer_name": "Industrial Pump Solutions",
                "match_score": 0.85,
                "method": "fuzzy",
            },
            "lines": [
                ...
                {
                    "line_no": 2,
                    "supplier_part_id": "UNKNOWN-PART-99",
                    "item_id_p21": "",
                    "item_description": "Custom Valve Assembly",
                    "qty_ordered": 2,
                    "unit_price": 1240.00,
                    "unit_of_measure": "EA",
                    "crosswalk_match_score": 0.0,
                    "confidence": "red",
                },
            ],
            "created_at": datetime.utcnow().isoformat(),
        },
        {
            ...
            "header": {
                ...
                "buyer": "Unknown",
                "ship2_name": "NewCo Manufacturing",
                "ship2_add1": "123 Unknown St",
                "ship2_city": "Somewhere",
                "ship2_state": "CA",
                "ship2_zip": "99999",
                "ship2_country": "US",
                "customer_id_p21": "",
            ...
            "lines": [
                {
                    "line_no": 1,
                    "supplier_part_id": "MYSTERY-001",
                    "item_id_p21": "",
                    "item_description": "Unknown Part Description",
                    "qty_ordered": 100,
                    "unit_price": 50.00,
                    "unit_of_measure": "EA",
                    "crosswalk_match_score": 0.0,
    ...
    for po in demo_pos:
        local_store.save_po(po["intake_id"], po)
    return {"status": "loaded", "count": len(demo_pos)}
...
class CustomerCrosswalkEditRequest(BaseModel):
...
@app.post("/api/v1/crosswalk/customers/{p21_customer_id}/edit")
async def edit_customer_crosswalk(
    p21_customer_id: str,
    req: CustomerCrosswalkEditRequest,
    request: Request,
    _auth=Depends(_require_api_key),
):
    """Edit a customer crosswalk row and write back to CSV."""
    global _customer_engine
    cw_dir = settings.crosswalk_dir
    csv_path = os.path.join(cw_dir, "customer_crosswalk.csv")

    if not os.path.exists(csv_path):
        raise HTTPException(404, "customer_crosswalk.csv not found")
    ...
    found = False
    for row in rows:
        if row.get("p21_customer_id") == p21_customer_id:
            row["p21_customer_name"] = req.p21_customer_name
            row["ship2_name"] = req.ship2_name
            row["ship2_zip"] = req.ship2_zip
            row["source_system"] = req.source_system
            found = True
            break

    if not found:
        raise HTTPException(404, f"Customer {p21_customer_id} not found in crosswalk")
    ...
    actor = request.headers.get("x-actor-name", "system")
    await _log_audit("", "crosswalk_edit", actor, f"Edited customer {p21_customer_id}")

    return {"status": "updated", "p21_customer_id": p21_customer_id}
