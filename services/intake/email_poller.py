...
# Watermark file for persisting last poll time across restarts
...
async def _graph_api_request(
        ...
        try:
            async with session.request(method, url, headers=headers, json=json, data=data) as resp:
                if resp.status == 429:
                    ...
                    logger.warning(
                        f"Graph API 429 on {url}, retrying after {wait:.1f}s "
                ...
                if resp.status >= 500:
                    if attempt < max_retries:
                        ...
                        logger.warning(
                            f"Graph API {resp.status} on {url}, retrying in {wait:.1f}s "
                            f"(attempt {attempt + 1}/{max_retries})"
                        )
                        await asyncio.sleep(wait)
                        continue
            ...
            if attempt < max_retries:
                wait = (2 ** attempt) + random.uniform(0, 0.5)
                logger.warning(
                    f"Graph API request error ({type(exc).__name__}) on {url}, "
    ...
    raise Exception(f"Graph API request failed after {max_retries} retries: {url}")
...
class GraphClient:
    ...
    async def _ensure_token(self) -> str:
        ...
        if not all([TENANT_ID, CLIENT_ID, CLIENT_SECRET]):
            raise ValueError(
                "Missing Azure AD credentials. Set AZURE_TENANT_ID, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET"
            )
    ...
    async def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        return await _graph_api_request(self.session, method, url, headers=headers, **kwargs)

    async def get_messages(
        self, folder: str = "inbox", filter_query: Optional[str] = None, top: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    async def download_attachment(self, message_id: str, attachment_id: str) -> bytes:
        ...
        return await _graph_api_request(
            self.session, "GET", url, headers=headers, return_bytes=True
        )
    ...
    async def move_message(self, message_id: str, destination_folder: str) -> Dict[str, Any]:
        ...
        if not dest_folder_id:
            ...
            create_resp = await self._request(
                "POST", folder_endpoint, json={"displayName": destination_folder}
            )
        ...
        return await self._request(
            "POST", move_endpoint, json={"destinationId": dest_folder_id}
        )
...
class EmailPoller:
    ...
    def _read_watermark(self) -> Optional[datetime]:
        """Read the last-poll watermark from disk."""
        ...
        try:
            ...
            self.logger.warning(f"Failed to read watermark file: {exc}")
            return None

    def _write_watermark(self, dt: datetime) -> None:
        """Persist the last-poll watermark to disk."""
        try:
            os.makedirs(os.path.dirname(WATERMARK_FILE), exist_ok=True)
            with open(WATERMARK_FILE, "w", encoding="utf-8") as f:
                f.write(dt.isoformat())
        except Exception as exc:
            self.logger.error(f"Failed to write watermark file: {exc}")
    ...
    async def poll_once(self, client: GraphClient) -> List[EmailMessage]:
        ...
        # Use persisted watermark when available to avoid re-processing after restart
        watermark = self._read_watermark()
        if watermark:
            since = watermark.isoformat() + "Z"
            self.logger.info(f"Using watermark since: {since}")
        else:
            since = (datetime.utcnow() - timedelta(hours=24)).isoformat() + "Z"
            self.logger.info(f"No watermark found, using default 24h lookback: {since}")

        filter_query = f"receivedDateTime ge {since}"

        messages = await client.get_messages(folder=INBOX_FOLDER, filter_query=filter_query, top=20)
        ...
        for msg in messages:
            ...
            # Skip already processed (in-memory guard within a single process lifetime)
                ...
                for att_meta in attachments_meta:
                    if att_meta.get("@odata.type") == "#microsoft.graph.fileAttachment":
                        ...
                        try:
                            ...
                            self.logger.error(f"Failed to download attachment {att_name}: {exc}")
            ...
            self.logger.info(
                f"Found PO email: {subject} | Source: {classification.source} | "
                f"Format: {classification.format} | Confidence: {classification.confidence:.2f}"
            )

        # Persist watermark so we don't re-process on restart
    ...
    async def _default_process(self, email: EmailMessage, client: GraphClient):
        ...
        for att in email.attachments:
            ...
            try:
                ...
                self.logger.error(f"Failed to process attachment {att.name}: {exc}")
